...
# Nodes need permission to join the cluster and pull ECR images
...
resource "aws_eks_cluster" "main" {
  ...
  # Enable control plane logging - important for debugging
...
# AWS manages the EC2 instances - patching, replacement, scaling
resource "aws_eks_node_group" "main" {
  cluster_name    = aws_eks_cluster.main.name
  node_group_name = "${var.project_name}-${var.environment}-nodes"
  node_role_arn   = aws_iam_role.nodes.arn

  # Nodes go in PRIVATE subnets - never public
  subnet_ids = var.private_subnet_ids

  instance_types = [var.node_instance_type]

  # Use SPOT instances to save ~60% cost
...
# The SRE Agent needs this to call Bedrock
